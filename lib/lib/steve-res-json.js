  ...
  function create(config) {
    ...
    function handler(reqProto, resProto, http) {
      ...
      resProto.getJson = function (data, opts) {
        ...
        replacer = (config.debug || opts.debug) ? null : removeStack;
      ...
      resProto.json = function (data, opts) {
        ...
        if ('string' === typeof data && opts.stringified) {
          jsons = resProto.getJson('STEVE-TPL-XYZ-987', opts).split('"STEVE-TPL-XYZ-987"');
          jsons = [
              jsons[0]
            , data
            , jsons[1]
          ...
          jsons = [resProto.getJson(data, opts)];
        }
